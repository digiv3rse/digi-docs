...
The DIGI API makes it easy to retrieve NFT metadata, monitor on-chain events, and analyze on-chain sales activity.

:::note

✨ **You can test out the API without needing a key. It’s free to use right now** ✨

API Endpoint: https://api.digiv3rse.xyz/graphql

[SDK](./ddk): A development kit to use the API without needing to know GraphQL

[NFT Hooks](../developer-tools/nft-hooks/intro): A tool that makes NFT data retrieval even easier than the DDK

:::

## Key Features

- Sales info from on-chain marketplaces (DIGI, OpenSea, LooksRare etc.)
...
| DIGI            | 7777777 | Entire History |
| DIGI Goerli     | 999     | Entire History |
...
- The [DDK](./ddk) and [NFT Hooks](../developer-tools/nft-hooks/intro) allow anyone to build top on of the API without needing to know GraphQL.
...
You can get an API key [here](https://keys.api.digiv3rse.xyz/) if your needs require **greater than 120 requests per minute.**
