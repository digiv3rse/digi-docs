---
id: ddk
title: DiGi Dev Kit (DDK)
---

import Tabs from '@theme/Tabs'
import TabItem from '@theme/TabItem'

Get started building with the DiGi Development Kit (DDK)

---
The DDK makes it easy to access the DiGi API without having to know GraphQL.

Check out the [DDK intro guide](../guides/ddk-intro-guide) to see it in action.

:::note
It is possible to make up to `30` requests per minute without needing an API Key.
Please reach out to [@DIGIEngineering](https://twitter.com/DIGIEngineering) on Twitter if would like to request a key.

The max response size for all requests is **200** except for `ddk.search` which is capped at a max size of **50**.

[NFT Hooks](../developer-tools/nft-hooks/intro) is a tool that makes it even easier to fetch NFT data than the DDK.
:::

---

### Installing
View the source code for the DDK: [GH Repository](https://github.com/digiv3rse/ddk)
...
```bash
npm install @digiv3rse/ddk --save
```
...
### Instantiating
```js
import { DDK, DDKNetwork, DDKChain } from "@digiv3rse/ddk";

const networkInfo = {
    network: DDKNetwork.Ethereum,
    chain: DDKChain.Mainnet,
  }
...
const ddk = new DDK(args) // All arguments are optional
```
...
Direct message [@DIGIEngineering](https://twitter.com/DIGIEngineering) on Twitter for an API Key.
...
Note 🚨: All the inputs for the DDK are `objects`. 
...
<a name="DDK+markets"></a>

---

### floorPrice

Gets data on the cheapest available NFT across all DiGi marketplaces.
...
Gets NFTs that are active on the DiGi markets e.g. Buy Now, Offers, Auctions.
...
| marketQueryFilter.marketType | An enum specifying the version of DiGi the order is made on | V1Ask, V1BidShare, V1Offer, V2Auction, V3Ask
...
Gets sales data for any NFT across multiple marketplaces e.g. DiGi, OpenSea, LooksRare, Foundation, etc.
