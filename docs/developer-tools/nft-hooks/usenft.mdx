...
The same data can be fetched using the base [`DDKFetchStrategy`](./useNft#ddkfetchstrategy) for server-side or non-React.
...
```ts
import { useNFT } from '@digiv3rse/nft-hooks'
...
Fetching data using the base `DDKFetchStrategy` for server-side or non-React and has the same response shape as `useNFT`.

```ts
import { Strategies, Networks } from '@digiv3rse/nft-hooks'
