---
id: v3-approvals
title: Approve Tokens to DiGi
sidebar_position: 1
---

##### Learn the first steps to getting started building with DIGI

---
DIGI V3 provides different types of market functionality that allows anyone to get started building with NFTs.
However, before being able to interact with specific functionality certain approvals must be granted to DIGI.

![digiv3diagram](../../static/img/digi_v3_diagram.png)
...
The DIGI core contracts include **Token Transfer Helpers** and the **Module Manager**.
...
This first section will show you how to get set up to interact with the DIGI contracts using the `ethers` js library.

The ABIs for all DIGI V3 contracts can be found in the `@digiv3rse/v3` npm package.
You can use the following 2 packages to import the ABIs and typechain types (if using typescript):

`yarn add @digiv3rse/v3 ethers` or `npm install @digiv3rse/v3 ethers`
...
`@digiv3rse/v3/dist/addresses/1.json` change to `@digiv3rse/v3/dist/addresses/5.json` 
...
Tokens must first be approved to the correct Transfer Helper before being able to be used by DIGI V3.
A token approval to the DIGI Transfer Helpers will **only ever need to be done once per token contract per user address**.
...
:::note 
If a user is trying to make a purchase with **just ETH** then they don't need to approve Transfer Helpers or DIGI modules.
:::
...
```javascript
const erc721TransferHelperAddress = mainnetDiGiAddresses.ERC721TransferHelper;
...
You can check the user's allowance to the DIGI Transfer Helper by calling the `allowance` function and then set the approved amount by calling the `approve` function.
Note that DIGI is compatible with any ERC-20.
...
```javascript
const erc20TransferHelperAddress = mainnetDiGiAddresses.ERC20TransferHelper;
...
### Revoking Access
At some point, a user might want to revoke DIGI's ability to access their assets. 
Approvals to DIGI can be revoked at any time and there are two ways to do it.
...
If you would like the DIGI Transfer Helpers to still have access to your assets, 
...
##### Multiple Modules
``` javascript
const digiModuleAddresses = [mainnetDiGiAddresses.AsksV1_1, mainnetDiGiAddresses.OffersV1]; // Asks v1.1 and Offers v1.0
await moduleManagerContract.setBatchApprovalForModules(
    digiModuleAddresses, 
    false
);
...
Now that you have learned how to approve and revoke tokens to DIGI you are ready to begin interacting with the different DIGI V3 Modules.
