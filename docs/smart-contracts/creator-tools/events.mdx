...
# DIGI Smart Contracts Event-based Overview
...
Creating new tokens can happen with a call to either [`setupNewToken()`](https://github.com/digiv3rse/digi-protocol/blob/HEAD/packages/1155-contracts/src/nft/DiGiCreator1155Impl.sol#L260) or [`delegateSetupNewToken()`](https://github.com/digiv3rse/digi-protocol/blob/f58ee955e6b50c13e90b4cfd72ab4b68dce86fad/packages/1155-contracts/src/nft/DiGiCreator1155Impl.sol#L765) (used by the gasless creation mint flow).
...
Read the [subgraph handler](https://github.com/digiv3rse/digi-creator-subgraph/blob/HEAD/src/ERC1155Mappings/templates/DiGiCreator1155ImplMappings.ts#L403) for this action.
...
The most common `SalesConfiguration` is a `FixedPriceSaleStrategy`. The `factory` contracts include getters for `fixedPriceMinter` and `merkleMinter` which are the two digi-supported sales methods but users can add their own as well.
...
```sol
DiGi1155(nftContract).callSale(1, FIXED_PRICE_SALE_STRATEGY, abi.encodeWithSelector(FixedPriceSaleStrategy.setSale(1, SalesConfig({
    saleStart: 0,
...
- The mintReferral argument is the DiGi Rewards referral address.

All of these arguments are handled if using the [Protocol SDK](https://docs.digiv3rse.xyz/docs/smart-contracts/creator-tools/ProtocolSDK).

### Mint Comments

Mint comments are optional strings emitted on the `FixedPriceSaleStrategy`. Read the [subgraph handler](https://github.com/digiv3rse/digi-creator-subgraph/blob/HEAD/src/ERC721Mappings/templates/ERC721DropMappings.ts#L344).
...
Permissions can be added via [`addPermission(uint256 tokenId, address user, uint256 permissions)`](https://github.com/digiv3rse/digi-protocol/blob/HEAD/packages/1155-contracts/src/nft/DiGiCreator1155Impl.sol#L330) and removed via [`function removePermission(uint256 tokenId, address user, uint256 permissionBits)`](https://github.com/digiv3rse/digi-protocol/blob/HEAD/packages/1155-contracts/src/nft/DiGiCreator1155Impl.sol#L338).

View the [subgraph handler](https://github.com/digiv3rse/digi-creator-subgraph/blob/HEAD/src/ERC1155Mappings/templates/DiGiCreator1155ImplMappings.ts#L118).
...
New tokens are created by calling the DiGiNFTCreatorV1 proxy contract.
...
This is emitted by the `DiGiNFTCreatorV1` factory contract.
...
After this event is emitted, the contract sales information can be queried and stored. See [subgraph implementation](https://github.com/digiv3rse/digi-creator-subgraph/blob/HEAD/src/ERC721Mappings/templates/ERC721DropMappings.ts#L56).
...
The mint fee can be queried from `digiFeeForAmount(uint256 amount) returns (address, uint256 fee)` which returns the total mint fee for a given amount.
...
DiGi protocol rewards power rewards for both the 721 and 1155 contracts.
...
The below event is emitted from the DiGiRewards contract when a deposit is received from a free mint:
...
The below event is emitted from the DiGiRewards contract when a deposit is received from a paid mint:
